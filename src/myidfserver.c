...
main()
{
	...
	*/
	...
	serv_addr.sin_port		= htons(5436);  // port number
		...
		while (1) {
			...
			*/
			...
			else
			{
				...
				printf("%s\n",buf);
				if(buf[0]=='e'){// for non parametric command execution
					char s[1000];
					int i;
					s[0]=' ';
					for(i=1;buf[i]!='\0';i++){
				...
				else if(buf[0]=='p'){// for parametric command execution
					char s[1000];
					int i;
					s[0]=' ';
					for(i=1;buf[i]!='\0';i++){
